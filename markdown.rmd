...
1.  Jaki wpływ mają rabaty na zysk - czy większy rabat zawsze zwiększa sprzedaż kosztem rentowności?
...
Identyfikacja braków: Wykryto braki danych w kluczowych zmiennych takich jak Discount (20%), Profit (11%) oraz danych adresowych.
...
Logika biznesowa: Braki w nazwach miast i stanów uzupełniono na podstawie kodów pocztowych (Postal Code), wykorzystując relacje przestrzenne w danych.

Czas dostawy: Brakujące daty wysyłki wyliczono na podstawie mediany czasu dostawy dla danego trybu wysyłki i regionu.
...
{r} ggboxplot(dane, x = "Category", y = "Profit", color = "Category", palette = "jco", ylab = "Zysk (\$)", xlab = "Kategoria") + coord_cartesian(ylim = c(-200, 500))

plotmeans(Profit \~ Category, data = dane, xlab = "Kategoria", ylab = "Zysk", main="Wykres średnich z 95% CI")

dane %\>% group_by(Category) %\>% shapiro_test(Profit)

ggplot(dane, aes(sample = Profit, colour = Category)) + stat_qq() + stat_qq_line() + facet_wrap(\~Category) + theme_bw()

leveneTest(Profit \~ Category, data = dane)

oneway.test(Profit \~ Category, data = dane, var.equal = FALSE \# zakładamy brak jednorodności )

model \<- aov(Profit \~ Category, data = dane) post_hoc \<- TukeyHSD(model) print(post_hoc)

ggbetweenstats( data = dane, x = Category, y = Profit,\
type = "p", conf.level = 0.95, var.equal = FALSE, \# Welch ANOVA pairwise.display = "significant", package = "ggthemes", palette = "Tableau_10", title = "Porównanie zysków między kategoriami", caption = "Źródło: Superstore Dataset" ) + coord_cartesian(ylim = c(-100, 300))

## **5.2. ANOVA 2-czynnikowa - Region x Kategoria a rentowność**

{r} df \<- df %\>% mutate(Profit_Ratio = Profit / Sales)

Wizualizacja interakcji

ggplot(df, aes(x = Region, y = Profit, color = Category, group = Category)) + stat_summary(fun = mean, geom = "line") + stat_summary(fun = mean, geom = "point") + labs(title = "Wykres interakcji: Region vs Kategoria", y = "Średni Zysk") + theme_minimal()

Test statystyczny

res_aov2 \<- aov(Profit \~ Region \* Category, data = df) summary(res_aov2)

## **5.3. Model regresji - Co determuinuje zysk?**

{r} reg_model \<- lm(Profit \~ Sales + Discount + Quantity + Category, data = dane) summary(reg_model)

## **5.4. Testy korelacji Pearsona - Rabaty a Sprzedaż/Zysk**

{r} \# 1. Test korelacji: Rabat vs Sprzedaż cor_sales \<- cor.test(dane$Discount, dane$Sales, method = "pearson")

## **5.5. Test korelacji: Rabat vs Zysk**
