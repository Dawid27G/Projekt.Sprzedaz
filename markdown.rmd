...
3.1. Rozkład rabatów w transakcjach
...
Kluczowe obserwacje:
- **Dominacja transakcji bez rabatu:** Około 4800 transakcji (ponad 50%) odbyło się bez jakiegokolwiek rabatu (Discount = 0), co wskazuje, że firma posiada baze klientów gotowych płacić pełną cenę
...
- **Potencjał optymalizacji:** Niska częstotliwość rabatów pośrednich (5-15%) może wskazywać na brak elastycznej polityki rabatów, która mogłaby lepiej reagować na różne segmenty klientów
...
1. **Analiza CLV (Customer Lifetime Value)** - odpowiada na pytanie: czy rabaty przyciągają lepszych klientów?
2. **Regresja logistyczna** - potwierdza i kwantyfikuje, jaki dokładnie wpływ ma rabat na prawdopodobieństwo rentowności
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
# Dane z CLV
clv_0 <- df_clv %>% filter(discount_category == "Brak rabatu (0%)") %>% pull(avg_profit_per_customer)
clv_high <- df_clv %>% filter(discount_category == "Wysoki (>40%)") %>% pull(avg_profit_per_customer)
stick_0 <- df_clv %>% filter(discount_category == "Brak rabatu (0%)") %>% pull(avg_transactions_per_customer)
stick_high <- df_clv %>% filter(discount_category == "Wysoki (>40%)") %>% pull(avg_transactions_per_customer)
prof_high <- df_clv %>% filter(discount_category == "Wysoki (>40%)") %>% pull(profitability_rate)

# Dane z logistyki
discount_effect <- (1 - exp(coefficients['Discount']))*100

cat("
**1. RABAT NISZCZY RENTOWNOŚĆ KOMPLETNIE**
   - CLV: Klient bez rabatu = $", round(clv_0, 2), " zysku, klient z >40% rabatu = $", round(clv_high, 2), " (-", round(abs((clv_high/clv_0-1)*100)), "%)
...
✓ **NOWA STRATEGIA:**
   - Utrzymać klientów bez rabatu ($", round(clv_0, 2), " zysku na klienta)
...
**ILOŚĆ PIENIĘDZY NA STOLE:** W grupie >40% rabatów średni klient generuje straty zamiast zysku ($", round(clv_high, 2), "). Jeśli ta grupa to ~470 klientów rocznie, to jest SETKI TYSIĘCY ZŁOTYCH strat rocznie, które można uniknąć
")
```
