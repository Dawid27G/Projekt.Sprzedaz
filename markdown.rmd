...
## 3.1. Sprzedaż i zysk w czasie

Poniższy wykres prezentuje dynamikę sprzedaży i zysku w ujęciu miesięcznym na przestrzeni analizowanego okresu (2014–2017). Wykorzystanie dwóch osi Y pozwala na bezpośrednie porównanie zmian w obydwu zmiennych, mimo że operują na różnych skalach. Obserwacja tych trendów jest kluczowa dla odpowiedzi na pytanie badawcze #4: *"Jak zmieniały się sprzedaż i zysk w czasie – czy występuje sezonowość?"*

Wykres ujawnia:
- **Trendy ogólne:** Sprzedaż wykazuje tendencję wzrostową szczególnie od 2015 roku, podczas gdy zysk oscyluje wokół stałego poziomu
- **Sezonowość:** Wyraźne peaki sprzedaży w końcowych miesiącach roku (listopad-grudzień) oraz spadki sprzedaży w początkowych miesiącach roku (styczeń-luty) 
- **Niezgodność sprzedaży i zysku:** W niektórych okresach wysokiej sprzedaży obserwuje się spadek zysku, co sugeruje wpływ polityki rabatowej 
...
## 3.2. Sprzedaż według kategorii i segmentów klientów

Poniższy wykres prezentuje rozkład sprzedaży w podziale na trzy kategorie produktów (Meble, Artykuły biurowe, Elektronika) oraz trzy segmenty klientów (Consumer – konsumenci indywidualni, Corporate – przedsiębiorstwa, Home Office – pracownicy pracujący z domu). Analiza ta bezpośrednio odnosi się do pytania badawczego #2: *"Które kategorie i podkategorie produktów generują najwyższą sprzedaż, a które najwyższy zysk?"*

Kluczowe obserwacje:
- **Dominacja Consumer:** Segment konsumentów indywidualnych generuje około 40-50% sprzedaży w każdej kategorii, co wskazuje na znaczenie rynku detalicznego
- **Elektronika liderem:** Kategoria Technology pokazuje najwyższą sprzedaż (~830k), przy czym Corporate segment jest tutaj proporcjonalnie silniejszy niż w innych kategoriach
- **Różna struktura segmentów:** Podczas gdy meble i artykuły biurowe mają podobny podział segmentów, elektronika wykazuje inny rozkład – sugeruje to różne wzorce zakupów w zależności od typu produktu
...
## 3.3. Relacja sprzedaży i zysku na poziomie transakcji

Poniższy wykres prezentuje relację między sprzedażą a zyskiem na poziomie poszczególnych transakcji, uwzględniając kategorię produktu oraz wielkość zamówienia (reprezentowana przez rozmiar bąbelka). Zastosowanie logarytmicznej skali osi X pozwala lepiej wizualizować dane o dużym rozproszeniu wartości sprzedaży. Analiza ta bezpośrednio odpowiada na pytania badawcze #1 i #3: *"Jaki wpływ mają rabaty na zysk?"* oraz *"Czy istnieją regiony o wysokiej sprzedaży, ale niskiej (lub ujemnej) rentowności?"*

Kluczowe obserwacje:
- **Strefa strat:** Znaczna część transakcji w kategoriach Office Supplies i Furniture znajduje się poniżej osi x=0, co wskazuje na straty pomimo wygenerowanej sprzedaży
- **Asymetria kategorii:** Technology dominuje górną część wykresu (wysokie zyski), podczas gdy Office Supplies jest rozproszona, szczególnie w strefie ujemnego zysku
- **Brak liniowej zależności:** Rozproszenie punktów sugeruje, że sama wielkość sprzedaży nie gwarantuje zysku – decydujące znaczenie mają rabaty, marża produktu i inne czynniki
- **Wielkość bąbelka:** Większe bąbelki (wyższe ilości) nie zawsze odpowiadają lepszym wynikom finansowym, co wskazuje na problem z polityką rabatową na produkty w dużych ilościach
...
p3 <- plot_ly(
  ...
  mode = "markers",
  color = ~Category,
  # Zamiast size = ~Quantity tutaj, robimy to w markerze poniżej:
  marker = list(
    size = ~Quantity * 2, # Ręczne przeskalowanie (dostosuj mnożnik)
    sizemode = 'diameter',
    line = list(width = 0), # Całkowite wyłączenie obramowania usuwa problem line.width
    opacity = 0.6
  ),
  text = ~paste0(
...
p3
```
## 3.4. Mapa cieplna sprzedaży: Kategoria × Region

Poniższa wizualizacja prezentuje rozkład sprzedaży w podziale na kategorie produktów i regiony geograficzne. Intensywność koloru (od jasnego do ciemnego niebieskiego) odzwierciedla wielkość sprzedaży, co pozwala na szybką identyfikację kombinacji kategoria-region o największym udziale w przychodach. Analiza ta jest kluczowa dla odpowiedzi na pytanie badawcze #3: *"Czy istnieją regiony o wysokiej sprzedaży, ale niskiej (lub ujemnej) rentowności?"*

Kluczowe obserwacje:
- **Koncentracja na East i West:** Regiony Wschodnia i Zachodnia generują zdecydowanie większość sprzedaży we wszystkich kategoriach, co wskazuje na słabą penetrację rynku w regionach Central i South
- **Technology dominuje na East i West:** Elektronika wykazuje najwyższe wartości sprzedaży w obu tych regionach, szczególnie na Wschodzie
- **Furniture preferuje West:** Meble osiągają najwyższą sprzedaż w regionie Zachodnim, co może sugerować różne preferencje konsumentów w zależności od lokalizacji
- **Office Supplies rozproszone:** Artykuły biurowe mają bardziej równomierny rozkład, ale również słabsza obecność w regionach Central i South
- **Szansa dla Central i South:** Regiony Central i South wykazują potencjał rozwojowy – najjaśniejsze komórki na mapie, co sugeruje niedostateczne zaangażowanie sprzedażowe
...
## 3.5. Rozkład zysku według Segmentu Klientów

Poniższy wykres violin plot prezentuje rozkład zysku dla trzech segmentów klientów: Consumer, Corporate i Home Office. Wizualizacja łączy informacje o rozkładzie danych (kształt wiolinu), medianie, kwartylach (box plot) i średniej (linia). Analiza ta pozwala zidentyfikować, które segmenty klientów są bardziej rentowne i w którym segmencie występuje największa zmienność wyników. Dane odpowiadają pytaniu badawczemu #2: *"Które segmenty klientów generują najwyższy zysk?"*

Kluczowe obserwacje:
- **Corporate segment najrentowniejszy:** Segment korporacyjny wykazuje wyraźnie wyższy medianę i średnią zysku niż dwa pozostałe segmenty
- **Home Office bardziej rentowny niż Consumer:** Wbrew intuicji, pracownicy pracujący z domu generują przeciętnie wyższe zyski niż konsumenci indywidualni
- **Wszechobecne straty:** Wszystkie trzy segmenty mają transakcje ze znacznymi stratami (do -6000$), co wskazuje na problem z polityką rabatową niezależnie od segmentu
- **Zmienność zysków:** Home Office wykazuje największą zmienność zysków (najszerszy rozkład), podczas gdy Corporate jest bardziej skoncentrowany wokół wyższych wartości
- **Potencjał optymalizacji:** Znaczna liczba transakcji zerowych lub nieznacznie zyskowych sugeruje możliwość poprawy marż przez lepsze zarządzanie rabatami
