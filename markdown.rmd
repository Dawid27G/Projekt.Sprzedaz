...
p_rabaty_region <- plot_ly(
  ...
  hovertemplate = paste(
    ...
    "Śr. rabat: %{x:.1f}%<br>",
    "Marża: %{y:.1f}%<br>",
    "Śr. sprzedaż: $%{customdata:.2f}<br>",
    "Zamówienia: %{customdata[1]:,}<extra></extra>"
  ),
  customdata = ~matrix(c(średnia_sprzedaż, liczba_zamówień), ncol = 2)
) %>%
  layout(
    title = list(
    ...
    annotations = list(
      list(
        ...
        text = "<b>Central i South: Najwyższe rabaty, najniższe marże</b>",
        showarrow = FALSE, font = list(size = 14, color = "red")
      )
    )
  )
...
```
**Silna negatywna zależność między rabatem a zyskiem**

Analiza 11 005 transakcji wykazała **istotną statystycznie ujemną korelację** między wysokością rabatu a zyskiem (ρ = -0,54). Przedział ufności Cl₉₅% [-0,55, -0,52] potwierdza stabilność tej relacji.

**Kluczowy wniosek:** Wyższe rabaty prowadzą do istotnego spadku zysku. Każdy dodatkowy procent rabatu zmniejsza rentowność transakcji.
...
Test Kruskala-Wallisa potwierdził istotne różnice w marżach między regionami (p ≈ 0):
- **West**: mediana = 29,0% – najwyższa
- **East**: mediana = 28,0%
- **South**: mediana = 26,25%
- **Central**: mediana = 19,0% – najniższa
